...
@testset "Workload Distribution" begin
    ...
    @testset "Scotch" begin
        ...
        @testset "$(join(N, '×')) - $(join(block_size, '×')) - $nghost" for (N, block_size, nghost) in (
            ...
            # By default only the best partition from 10 retries will be kept. We should always reach
            # the optimum with those options.
            ref_params = get_reference_params(:Sod, Float64; nghost, N, block_size, workload_distribution=:scotch)
            @testset "Default - parts=$parts" for parts in (1, 4, 7, 59, 64)
                check_distribution(ref_params, parts)
            end

            @testset "Repart - parts=$parts" for parts in (1, 4, 7, 59, 64)
                check_distribution(ref_params, parts; repart=true)
            end
